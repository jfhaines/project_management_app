...
var ColumnWhere = struct {
	...
	ID:      whereHelperint{field: "`columns`.`id`"},
	Name:    whereHelperstring{field: "`columns`.`name`"},
	BoardID: whereHelperint{field: "`columns`.`board_id`"},
}
...
var (
	...
	columnColumnsWithoutDefault = []string{"name", "board_id"}
	columnColumnsWithDefault    = []string{"id"}
...
func FindColumn(exec boil.Executor, iD int, selectCols ...string) (*Column, error) {
	columnObj := &Column{}
...
func (o *Column) Insert(exec boil.Executor, columns boil.Columns) error {
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == columnMapping["id"] {
		goto CacheNoHooks
	}
...
func ColumnExists(exec boil.Executor, iD int) (bool, error) {
	var exists bool
