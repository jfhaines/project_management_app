...
var CardCommentWhere = struct {
	...
	ID:     whereHelperint{field: "`card_comments`.`id`"},
	Text:   whereHelperstring{field: "`card_comments`.`text`"},
	Time:   whereHelpertime_Time{field: "`card_comments`.`time`"},
	UserID: whereHelperint{field: "`card_comments`.`user_id`"},
	CardID: whereHelperint{field: "`card_comments`.`card_id`"},
}
...
var (
	...
	cardCommentColumnsWithoutDefault = []string{"text", "time", "user_id", "card_id"}
	cardCommentColumnsWithDefault    = []string{"id"}
...
func FindCardComment(exec boil.Executor, iD int, selectCols ...string) (*CardComment, error) {
...
func (o *CardComment) Insert(exec boil.Executor, columns boil.Columns) error {
	...
	o.ID = int(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == cardCommentMapping["id"] {
		goto CacheNoHooks
	}
...
func CardCommentExists(exec boil.Executor, iD int) (bool, error) {
	var exists bool
